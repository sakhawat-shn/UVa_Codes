...
LLD dp_count(int val,int stp)
{
    if(val<0||stp<0) return 0;
    if(val==0&&stp==0) return 1;
    if(dp[val][stp]>=0) return dp[val][stp];
    LLD sum=0;

    for(int i=0;i<=val;i++)
    {
        sum=(sum+dp_count(val-i,stp-1))%1000000;
    }
    return dp[val][stp]=(sum%1000000);
}

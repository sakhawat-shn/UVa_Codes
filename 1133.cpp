...
int dp(int n)
{
    if(n==vec_size) return 0;
    if(vti[n]!=-1) return vti[n];
    int res=-1;
    int start=get<0>(vt[n]);
    int req_time=get<1>(vt[n]);
    int vara=get<2>(vt[n]);

    //nilam....
    res=vara;
    for(int i=n+1;i<vec_size;i++)
    {
        if((start+req_time)<=(get<0>(vt[i])))
        {
            res=res+dp(i);
            break;
        }
...
int main()
{
    ...
    while(scanf("%d",&n)==1)
    {
        if(flag)print(newline);
        flag=1;
        for(int i=0; i<n; i++)
        {
            scanf("%s %d %d %d",st,&s,&d,&pr);
            vt.pb(make_tuple(s,d,pr));
            vti.pb(-1);
        }

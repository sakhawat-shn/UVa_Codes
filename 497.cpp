...
int main()
{
    ...
    for(int i=0; i<t; i++)
    {
        ...
        while(scanf("%[^\n]",numb)==1)
        {
            vc.push_back(extract((string)numb,i));
            scanf("%c",&ch);
        }
        scanf("%c",&ch);
        ...
        for(int j=0; j<sz; j++)
        {
            ...
            if(V.end()==iv)
            {
                ...
                {
                    vc_pos.push_back(V_pos[V_pos.size()-2]);
                }
            }
            else
            {
                ...
                {
                    vc_pos.push_back(V_pos[iv-V.begin()-1]);
                }
            }
        }
        printf("Max hits: %d\n",V.size());
        ...
        vector<int> result;
        result.push_back(V_pos[V_pos.size()-1]);
        int now_point=result[result.size()-1];
        while(vc_pos[now_point]!=now_point)
        {
            result.push_back(vc_pos[now_point]);
            now_point=result[result.size()-1];
        }

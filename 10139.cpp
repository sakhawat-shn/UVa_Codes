...
vector<pair<int,int> > primeFctors(LLD n)
{
    vector<pair<int,int> > factors;
    LLD pf_idx=0,pf=primes[pf_idx];
    int num=0;
    while(pf*pf<=n)
    {
        while(n%pf==0)
        ...
        if(num!=0){factors.push_back({(int)pf,num});}
        num=0;
        pf=primes[++pf_idx];
    }
...
int times(LLD n,LLD x)
{
    int ret=0;
    for(LLD i=x;i<=n;i=i*x)
    {
        ret=ret+(int)(n/i);
    }
    return ret;
...
int finder(LLD a,LLD b)
{
    vector<pair<int,int> > factors;
    factors=primeFctors(b);
    int sz=factors.size();
    for(int i=0;i<sz;i++)
    {
        if(factors[i].second>times(a,factors[i].first))
        {
            return 0;
        }
    }

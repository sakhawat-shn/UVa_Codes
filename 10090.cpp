...
void finder(LLD a,LLD b,LLD c1,LLD c2)
{
    LLD mn=(LLD)ceil(((-x)*(double)d)/b);
    LLD mx=(LLD)floor((y*(double)d)/a);
    LLD min_pr=c1*(x+(b/d)*mn)+c2*(y-(a/d)*mn),
        max_pr=c1*(x+(b/d)*mx)+c2*(y-(a/d)*mx);

...
int main()
{
    ...
    for(int i=0;i<t;i++)
    {
        ...
        scanln(st);
        int sz=st.size();
        vector<pair<LLD,char> > fib_char;
        for(int j=0,k=0;j<sz&&k<i_list.size();j++)
        {
            if(is_capi(st[j]))
            {
                fib_char.pb({i_list[k],st[j]});
                k++;
            }
        }
        sort(fib_char.begin(),fib_char.end());
        sz=fib_char.size();
        LLD a=1,b=1,k=0;
        while(k<sz)
        {
            if(fib_char[k].first==b)
            {
                print(fib_char[k].second);
                k++;
            }
            else
            {
